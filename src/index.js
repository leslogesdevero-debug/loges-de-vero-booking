...
async function getTarifs(env, request) {
  try {
    ...
    console.log('DEBUG getTarifs — CONFIG_KV.get résultat:', fromKV ? 'trouvé' : 'null/vide');
  ...
  console.log('DEBUG getTarifs — repli sur le fichier statique');
...
async function handleAdminSaveTarifs(request, env) {
  ...
  try {
    ...
    console.log('DEBUG handleAdminSaveTarifs — écriture KV terminée, duplex.nightlyDefault =', updated.duplex.nightlyDefault);
